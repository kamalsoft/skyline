    ...
    return (
        <VStack align="stretch" position="relative">
            ...
            {(results.length > 0 || error) && (
                ...
                >
                    {error ? (
                        <Text color="red.500" p={2}>{error}</Text>
                    ) : (
                        <List spacing={1}>
                            {results.map((location) => (
                                <ListItem key={location.id} p={2} borderRadius="md" _hover={{ bg: hoverBgColor, cursor: 'pointer' }} onClick={() => handleSelect(location)}>
                                    <HStack justify="space-between">
                                        <Text fontWeight="bold">{location.name}, {location.country}</Text>
                                        <Text fontSize="sm" color="gray.500">{location.admin1}</Text>
                                    </HStack>
                                </ListItem>
                            ))}
                        </List>
