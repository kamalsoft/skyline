...
function LogEntry({ log }) {
  ...
  return (
    <HStack align="flex-start">
      <Text color="gray.500" whiteSpace="nowrap" minW="80px">
        {log.timestamp}
      </Text>
      <Text
        color="purple.400"
      ...
      <Text color={textColor} whiteSpace="pre-wrap" wordBreak="break-word" flex="1">
        {log.message}
      </Text>
    </HStack>
  );
      ...
      >
        ...
        >
          ...
          <ButtonGroup size="xs" isAttached variant="outline" pt={1} onPointerDown={(e) => e.stopPropagation()}>
            <Button onClick={() => setFilterLevel('all')} isActive={filterLevel === 'all'}>
              All
            </Button>
            <Button onClick={() => setFilterLevel('info')} isActive={filterLevel === 'info'}>
              Info
            </Button>
            <Button onClick={() => setFilterLevel('warn')} isActive={filterLevel === 'warn'}>
              Warn
            </Button>
            <Button onClick={() => setFilterLevel('error')} isActive={filterLevel === 'error'}>
              Error
            </Button>
          </ButtonGroup>
        </Box>
        <Box ref={terminalRef} p={3} fontFamily="monospace" fontSize="sm" overflowY="auto" flex="1">
