...
function AnalogClock({ time, clockTheme }) {
  ...
  const numberFontFamily = useColorModeValue(
    `${clockTheme}.light.numberFontFamily`,
    `${clockTheme}.dark.numberFontFamily`
  );
  ...
  return (
    <Box
      ...
      boxShadow={
        clockTheme === 'metallic'
          ? `inset 5px 5px 10px ${shadowDark}, inset -5px -5px 10px ${shadowLight}, 3px 3px 8px rgba(0,0,0,0.4)`
          : 'md'
      }
