...
function DetailedWeatherModal({ isOpen, onClose, data, displayTemp }) {
    ...
    const renderHourlyDetails = () => (
        <VStack spacing={3} align="stretch">
            ...
            <Text><strong>Wind:</strong> {details.wind_speed_10m} km/h from {details.wind_direction_10m}°</Text>
        </VStack>
    );
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} isCentered>
            ...
            <ModalContent className="glass">
                <ModalHeader>
                    Detailed Forecast for {details.time}
                </ModalHeader>
                <ModalCloseButton />
                <ModalBody>
                    <VStack spacing={4} align="stretch">
                        <HStack justify="space-around">
                            <Text fontSize="2xl" fontWeight="bold">{details.temperature}</Text>
                        </HStack>
                        <Divider />
                ...
                <ModalFooter>
                    <Text fontSize="xs" color="gray.500">Weather data from Open-Meteo</Text>
                </ModalFooter>
            </ModalContent>
        </Modal>
    );
