...
function SunCalendar({ latitude, longitude }) {
    ...
    return (
        <Box p={4}>
            ...
            {loading ? (
                <VStack justify="center" minH="200px"><Spinner /></VStack>
            ) : (
                <Grid templateColumns="repeat(7, 1fr)" gap={2}>
                    {weekdays.map(day => <Text key={day} fontWeight="bold" textAlign="center" fontSize="sm">{day}</Text>)}
                    {/* Render empty boxes for days before the 1st of the month */}
                    {Array.from({ length: firstDayOfMonth }).map((_, i) => <Box key={`empty-${i}`} />)}
                    {sunData.time && sunData.time.map((day, index) => {
                        const dayDate = new Date(day);
                        const isToday = dayDate.getFullYear() === today.getFullYear() && dayDate.getMonth() === today.getMonth() && dayDate.getDate() === today.getDate();
                        return (
                            <VStack key={day} p={2} className="glass" borderRadius="lg" spacing={1} border={isToday ? "2px solid" : "1px solid"} borderColor={isToday ? "accentPink" : "transparent"}>
                                <Text fontWeight="bold" fontSize="md">{dayDate.getDate()}</Text>
                                <HStack>
                                    <Box as={WiSunrise} size="20px" color="yellow.400" />
                                    <Text fontSize="xs">{new Date(sunData.sunrise[index]).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text>
                                </HStack>
                                <HStack>
                                    <Box as={WiSunset} size="20px" color="orange.400" />
                                    <Text fontSize="xs">{new Date(sunData.sunset[index]).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Text>
                                </HStack>
                            </VStack>
                        );
                    })}
